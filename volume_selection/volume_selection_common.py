...
"""
This common should be put together with scatter selection common, with differences commented in this code.
"""

class SelectionCommon(object):
    def __init__(self, canvas, view, vol_data, volume, volume_pool, pos_data):
        self._canvas = canvas
        self._view = view
        ...
        self.tr = self._volume.node_transform(self._view)
    ...
    def on_mouse_press(self, event):
        ...
        if event.button == 1 and self.selection_flag:
            if self.selection_id == '4':
                ...
                data = self.tr.map(self._vol_data)[:, :2]
                m1 = data > (event.pos - 4)
                m2 = data < (event.pos + 4)
    ...
    def on_mouse_release(self, event):
        ...
        if event.button == 1 and self.selection_flag and self.selection_id is not '4':
            # self.facecolor[self.facecolor[:, 1] != 1.0] = self.white  # What different from scatter selection

            data = self.tr.map(self._pos_data)[:, :2]   # What different from scatter selection

            if self.selection_id in ['1', '2', '3']:
                selection_path = path.Path(self.line_pos, closed=True)
                mask = selection_path.contains_points(data)  # What different from scatter selection
